...
impl Ticket {
    fn verify(title: &str, description: &str, status: &str) {
        if title.is_empty() {
    ...
    pub fn new(title: String, description: String, status: String) -> Ticket {
        Ticket::verify(&title, &description, &status);
    ...
    pub fn set_title(&mut self, title: String) {
        Ticket::verify(&title, &self.description, &self.status);
        self.title = title;
    }

    pub fn set_description(&mut self, desc: String) {
        Ticket::verify(&self.title, &desc, &self.status);
        self.description = desc;
    }

    pub fn set_status(&mut self, status: String) {
        Ticket::verify(&self.title, &self.description, &status);
        self.status = status;
    }
}
