...
async fn printer<T>(listener: TcpListener, reply: Arc<T>)
where
    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(printer(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(printer(second, Arc::clone(&reply)));
